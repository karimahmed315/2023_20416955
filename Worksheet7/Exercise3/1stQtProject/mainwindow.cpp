...
void MainWindow::on_actionOpen_File_triggered()
{
    ...
    QString filename = QFileDialog::getOpenFileName(this, tr("Open File"),"C:\\",tr("STL Files(*.stl);;Text Files(*.txt)"));
        ...
        emit statusUpdateMessage("The selected file to open is: " + filename
            , 0);
    // Create a new item in the tree view (assuming you have a QTreeWidget named 'treeWidget')
    QTreeWidgetItem* newItem = new QTreeWidgetItem();
    newItem->setText(0, filename); // Set the text to the filename

    // Add the new item as a child of the currently selected item (if any)
